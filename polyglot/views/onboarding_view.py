...
class OnboardingView(BaseView):
    ...
    def setup_ui(self):
        ...
        self.title = ctk.CTkLabel(
            self, text="Welcome to Polyglot!", font=("Helvetica", 24, "bold")
        )
    ...
    def show_level_selection(self):
        ...
        label = ctk.CTkLabel(
            self.content, text="Select your proficiency level:", font=("Helvetica", 16)
        )
        label.pack(pady=10)

        for level in self.levels:
            btn = ctk.CTkButton(
                self.content, text=level, command=lambda l=level: self.select_level(l)
            )
            btn.pack(pady=5)
